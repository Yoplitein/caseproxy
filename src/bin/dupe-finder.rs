...
use std::{collections::{HashMap, VecDeque}, fmt::Write, io::Read, path::{Path, PathBuf}};
...
fn find_all_files(root: &Path) -> AResult<Vec<PathBuf>> {
	...
	while !queue.is_empty() {
		let Some(dir) = queue.pop_front() else { unreachable!() };
...
fn print_text_report(duplicateSets: &HashMap<InsensitivePath, Vec<PathBuf>>, hashes: &HashMap<PathBuf, String>) {
	for (path, instances) in duplicateSets {
		println!("{:?}", path.0);
		for instance in instances {
			let hash = hashes.get(instance).map(String::as_str).unwrap_or("missing");
			println!(" => {instance:?} {hash}");
		}
	}
}

fn create_html_report(duplicateSets: &HashMap<InsensitivePath, Vec<PathBuf>>, hashes: &HashMap<PathBuf, String>) -> AResult<String> {
	let mut res = String::new();
	writeln!(&mut res, "<style>")?;
	writeln!(&mut res, "table {{ border-collapse: collapse; width: 100%; }}")?;
